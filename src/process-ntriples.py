...
def yield_rows(input: list[str], prefix_map: dict[str, str]) -> Iterator[tuple[str,str,str,str]]:
                ...
                for line in tinf:
                    ...
                    for prefix in prefix_map:
                        if s.startswith(prefix):
                            s = s.replace(prefix, prefix_map[prefix], 1)[:-1]
                        if p.startswith(prefix):
                            p = p.replace(prefix, prefix_map[prefix], 1)[:-1]
                        if not object_is_literal and o.startswith(prefix):
                            o = o.replace(prefix, prefix_map[prefix], 1)[:-1]
                        if d.startswith(prefix):
                            d = d.replace(prefix, prefix_map[prefix], 1)[:-1]
                    yield (s, p, o, d)
...
def yield_batches(input: list[str], prefix_map: dict[str, str], parquet_batch_size: int, schema: pa.Schema) -> Iterator[pa.RecordBatch]:
    batch = []
    for row in yield_rows(input, prefix_map):
        batch.append(row)
...
@click.command()
@click.option("-p", "--prefixes", help="prefix CSV file", required=False, type=click.Path(dir_okay=False, readable=True))
...
def convert_ntriples(input: list[str], prefixes: str, output: str, max_file_size: int) -> None:
    ...
    ds.write_dataset(yield_batches(input, prefix_map, 1024*1024, schema), output+".tmp", format='parquet', partitioning_flavor="hive", partitioning=["property"], schema=schema, min_rows_per_group=2**16, file_options=ds.ParquetFileFormat().make_write_options(compression='zstd'))
