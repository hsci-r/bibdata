...
dmtime <- if (params$date_modified!='') params$date_modified else strftime(fs::file_info(Sys.glob(params$dataset)[[1]])$modification_time,"%Y-%m-%d")
d_a <- open_dataset(Sys.glob(params$dataset))
d <- d_a |> to_duckdb()
...
`r p(n_records)` records in total.
