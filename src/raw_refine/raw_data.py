...
for row in formats.collect(backend='polars').iter_rows(named=True):
    ...
    if dataset == "wikidata":
        for subdataset in {re.sub(r"(_[0-9])*\.parquet","", file.name) for file in here("data/wikidata").glob("*.parquet")}:
            ...
            globals()[f"wd_{clean_subdataset}"] = read_parquet(f"wd_{clean_subdataset}", here(f"data/wikidata/{subdataset}*.parquet"))
            #print(f"wd_{clean_subdataset} = cast(nw.LazyFrame[DuckDBPyRelation], None)")
    elif dataset == "isni":
        for subdataset in {re.sub(r"(_[0-9])*\.parquet","", file.name) for file in here("data/isni").glob("*.parquet")}:
            globals()[f"isni_{subdataset}"] = read_parquet(f"isni_{subdataset}", here(f"data/isni/{subdataset}*.parquet"))
            #print(f"isni_{subdataset} = cast(nw.LazyFrame[DuckDBPyRelation], None)")
    elif dataset == "geonames":
        globals()["geonames"] = read_parquet("geonames", here("data/geonames/geonames*.parquet"))
        globals()["geonames_alternate_names"] = read_parquet("geonames_alternate_names", here("data/geonames/alternate_names*.parquet"))
        #print(f"geonames = cast(nw.LazyFrame[DuckDBPyRelation], None)")
        #print(f"geonames_alternate_names = cast(nw.LazyFrame[DuckDBPyRelation], None)")
    elif standard == "rdf":
