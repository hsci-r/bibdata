...
def to_narwhals(duckdb_relation: duckdb.DuckDBPyRelation) -> nw.LazyFrame:
    return nw.from_native(duckdb_relation)

def to_duckdb(lnf: nw.LazyFrame) -> duckdb.DuckDBPyRelation:
    return lnf.to_native()

def read_parquet(table_name: str, glob: Path) -> nw.LazyFrame:
    con.sql(f"CREATE OR REPLACE VIEW {table_name} AS SELECT * FROM read_parquet('{glob}');")
    return to_narwhals(con.view(table_name))

def to_table(table_name: str, lnf: nw.LazyFrame, temporary: bool = False, replace: bool = True) -> nw.LazyFrame:
    if replace:
...
def to_parquet(table_name: str, path: Path, lnf: nw.LazyFrame, *args, **kwargs) -> nw.LazyFrame:
    to_duckdb(lnf).write_parquet(str(path), compression='zstd', *args, **kwargs)
    return read_parquet(table_name, path)

def to_df(lnf: nw.LazyFrame):
...
__all__ = ["con", "to_narwhals", "to_duckdb", "read_parquet", "to_table", "to_parquet", "to_df", "format_sql", "here", "tqdm", "duckdb", "nw", "cast"]